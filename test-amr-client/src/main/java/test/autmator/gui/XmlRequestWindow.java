...
public class XmlRequestWindow {
	...
	private void initialize() {
		...
		JScrollPane scroll1 = new JScrollPane(reqXMLTextField, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
		...
		JScrollPane scroll2 = new JScrollPane(resXMLTextField, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
			...
			if (cmd.equals("Next")) {
				...
				if (reqXML.equals("") && resXML.equals("")) {
					...
					int userOption = JOptionPane.showOptionDialog(null,
							"Request XML and Response XML\nfields contain same file!", "Confirm", 0, 3, null, options,
							options[0]);
					if (userOption == 0) {
						// back
		...
		chckbxRemoverepeatednodes.addItemListener(e -> removeRepestedNodes = e.getStateChange() == 1);
		...
		chckbxUpdateAutomatorOutput.addItemListener(e -> Constants.UPDATE_NEWFILES_NAMES = e.getStateChange() == 1);
